...
class SelfBuffHandler(BaseHandler):
    ...
    def __init__(self, keyboard, farm_logic, pausable_logic):
        super().__init__(keyboard)
        self.farm_logic = farm_logic
        self.pausable_logic = pausable_logic
    ...
    def handle_state(self, last_action_delta):
        if self.current_state == STATE_IDLE and last_action_delta > 18 * 60 and not self.farm_logic.has_target:
            self.write_log("BUFF", "Rebuff in progress")
            for logic in self.pausable_logic:
                logic.pause()
